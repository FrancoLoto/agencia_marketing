                ...
                <div className="col-span-9">
                    ...
                    <div className="relative">
                            <div className="relative -mb-6 w-full overflow-x-auto pb-6">
                                ...
                                >
                                    <Link to='/blog'
                                    className={`${location.pathname === '/blog' ? "text-indigo-500 bg-white":"text-gray-900 hover:text-orange-500 border border-gray-100 hover:border-gray-200"} py-2 px-6   rounded-md text-lg font-regular`}>
                                        All
                                    </Link>
                                    {
                                        categories&&categories.map((category,index)=>(
                                            <Link key={index} to={`/category/${category.slug}`}
                                            className={`${location.pathname === `/category/${category.slug}` ? "text-indigo-500 bg-white":"text-gray-900 border border-gray-100 hover:border-gray-200"} py-2 px-6 rounded-md text-lg font-regular`}>
                                                {category.name}
                                            </Link>
                                        ))
                                    }
